...
const loadData = async () => {
  ...
  try {
    ...
    if (props.modelValue != null && !inputDisplayText.value) {
      ...
      if (hit) inputDisplayText.value = String(hit[props.labelKey] ?? '')
    }
  } catch {
...
const selectRow = (row: ReferRow) => {
  ...
  inputDisplayText.value = String(row[props.labelKey] ?? '')
...
<template>
  ...
  >
    <template #suffix>
      <el-icon v-if="inputDisplayText && !disabled" class="refer-suffix-icon" @click.stop="clearValue">
        <CircleClose />
      </el-icon>
      <el-icon v-if="!disabled" class="refer-suffix-icon" @click.stop="openDialog">
        <OfficeBuilding />
      </el-icon>
    </template>
  </el-input>
