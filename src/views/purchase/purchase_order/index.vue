...
const queryData = reactive<PurchaseOrderQuery>({
  pageNum: 1,
  pageSize: 10,
  purchaseOrderNo: '',
  materialId: '',
...
const statusOptions = [
  { label: '草稿', value: PurchaseOrderStatus.DRAFT, tone: 'info' },
  { label: '已审批', value: PurchaseOrderStatus.APPROVED, tone: 'primary' },
  { label: '已发货', value: PurchaseOrderStatus.SHIPPED, tone: 'warning' },
  { label: '已收货', value: PurchaseOrderStatus.RECEIVED, tone: 'success' },
  { label: '已关闭', value: PurchaseOrderStatus.CLOSED, tone: 'danger' },
] as const
...
const workflowMap: Partial<Record<number, { label: string; next: number }>> = {
  [PurchaseOrderStatus.DRAFT]: { label: '审批订单', next: PurchaseOrderStatus.APPROVED },
  [PurchaseOrderStatus.APPROVED]: { label: '确认发货', next: PurchaseOrderStatus.SHIPPED },
  [PurchaseOrderStatus.SHIPPED]: { label: '确认收货', next: PurchaseOrderStatus.RECEIVED },
  [PurchaseOrderStatus.RECEIVED]: { label: '关闭订单', next: PurchaseOrderStatus.CLOSED },
}
const workflow = computed(() => workflowMap[selectedRow.value?.status ?? -1] ?? null)
...
const loadData = async () => {
  ...
  tableData.value = await getPagePurchaseOrder(queryData)
}
...
const reset = () => {
  Object.assign(queryData, { pageNum: 1, purchaseOrderNo: '', materialId: '', supplierId: '', status: '' })
  loadData()
}

const changeStatusFilter = (status: string | number) => {
  queryData.status = Number(status) as PurchaseOrderQuery['status']
  query()
}
...
const openEdit = () => {
  ...
  if (selectedRow.value.status !== PurchaseOrderStatus.DRAFT) return void ElMessage.warning('仅草稿采购订单可编辑')
  saveMode.value = 'edit'
  saveVisible.value = true
}

const openDetail = (row: PurchaseOrderVo) => {
  detailId.value = row.id
  detailVisible.value = true
}

const submit = async (data: PurchaseOrderAdd | PurchaseOrderUpdate) => {
  if (saveMode.value === 'edit' && selectedRow.value)
    await updatePurchaseOrder(selectedRow.value.id, data as PurchaseOrderUpdate)
...
const advanceWorkflow = async () => {
  ...
  try {
    ...
    if (action.next === PurchaseOrderStatus.APPROVED) await approvePurchaseOrder(id)
    else if (action.next === PurchaseOrderStatus.SHIPPED) await shipPurchaseOrder(id)
    else if (action.next === PurchaseOrderStatus.RECEIVED) await receivePurchaseOrder(id)
...
<template>
  <div class="container">
    <ProPageHeader
      title="采购订单"
      description="管理供应商采购订单，跟踪审批、发货与收货进度"
    ...
    <section class="table">
      <ProTable
        ...
        "
        @selection-change="(rows: PurchaseOrderVo[]) => (selectedRow = rows[0] ?? null)"
